...
function showDayActivities(dateStr) {
    ...
    dayEvents.forEach(ev => {
        ...
        if (currentUser.role === "membre") {
            div.innerHTML += `
                <button class="btn-green" onclick="setParticipation('${ev.id}', 'participant')">JE PARTICIPE</button>
                <button class="btn-red" onclick="setParticipation('${ev.id}', 'indisponible')">INDISPONIBLE</button>
            `;
        }
        ...
        if (currentUser.role === "commandement") {
            div.innerHTML += `
                ...
                <ul>${ev.participants.map(u => `<li class="green">${u.grade} ${u.nom} (${u.login})</li>`).join("")}</ul>

                <h4>INDISPONIBLE (${ev.indisponibles.length})</h4>
                <ul>${ev.indisponibles.map(u => `<li class="red">${u.grade} ${u.nom} (${u.login})</li>`).join("")}</ul>

                <button class="btn-red" onclick="deleteEvent('${ev.id}')">SUPPRIMER LA MISSION</button>
            `;
        }
...
function createMission() {
    ...
    document.getElementById("mission-concerned").value = "";
}

function updateUserLabel() {
    const label = document.getElementById("user-label");
    if (label && currentUser) {
        const roleTxt = currentUser.role === "commandement" ? "COMMANDEMENT" : "MEMBRE";
        label.textContent = `${currentUser.grade} ${currentUser.nom} — ${roleTxt}`;
    }
}

// Appelée automatiquement par data.js à chaque mise à jour Firebase
// (nouvelle mission créée, quelqu'un répond présent/absent, etc.)
window.refreshUI = function() {
    renderCalendar();
    if (selectedDate) showDayActivities(selectedDate);
};
